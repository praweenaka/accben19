...
if($_GET["Command"]=="addchq_cash_rec")
{
		...
		$sql="insert into tmp_rec_ent(entno, accno, accname, descript, amt, tmp_no) values ('".$_GET["txt_entno"]."', '".$_GET["accno"]."', '".$_GET["acc_name"]."', '".$_GET["TXT_NARA"]."', ".$_GET["amt"].", '".$_SESSION["tmp_no_rec_ent_acc"]."')";
...
/*
	...
	while ($_GET["mcount"]>$i){
		...
		if($_GET["Command"]=="new_rec")
		{
			...
			$sql="SELECT recedirect FROM  dep_mas";
			$result=mysql_query($sql, $dbacc);
			$row = mysql_fetch_array($result);
			$tmprecno="000000".$row["recedirect"];
			$lenth=strlen($tmprecno);
			$recno=$_SESSION['company']."/".date("y")."/DRE/".substr($tmprecno, $lenth-7);
			...
			$tmprecno="000000".$row["recedirect"];
			$lenth=strlen($tmprecno);
			$_SESSION["tmp_no_rec_ent_acc"]=$_SESSION['company']."/".date("y")."/DRE/".substr($tmprecno, $lenth-7);
...
if($_GET["Command"]=="save_crec")
{
	...
	if ($_SESSION["txt_stat"] == "new") {
		...
		$sql="update  dep_mas set recedirect=recedirect+1";
		$result=mysql_query($sql, $dbacc);	
	} else {
 ...
 //com_Flex1.Visible = False
    if ($_SESSION["txt_stat"] == "edit") {
       if ($_GET["txt_entno"] != "") {
		  ...
		  $sql_rsrecemas="delete from paymas where refno='".trim($recno)."'";
		  $result_rsrecemas=mysql_query($sql_rsrecemas, $dbacc);
		  
		  $sql_rsrecemas="delete from cashpaytrn where refno='".trim($recno)."'";
		  $result_rsrecemas=mysql_query($sql_rsrecemas, $dbacc);
		  
		  $sql_rsledger="select * from ledger where l_refno='".trim($recno)."'";
		  $result_rsledger=mysql_query($sql_rsledger, $dbacc);
		  while ($row_rsledger = mysql_fetch_array($result_rsledger)){
		  	if (trim($row_rsledger["l_refno"]) == trim($recno)) {
		  ...
		  $sql_rsledger1="delete from ledger where l_refno='".trim($recno)."'";
		  $result_rsledger1=mysql_query($sql_rsledger1, $dbacc);
...
/*    if ($_GET["txt_stat"] == "new") {
	...
	$sql="Delete from  recmas where refno = '" . trim($recno) . "'";
	$result=mysql_query($sql, $dbacc);
	
	$sql="Delete from  ledger where l_refno = '" . trim($recno) . "'";
	$result=mysql_query($sql, $dbacc);
	
	$sql="Delete from  recpaytrn where refno = '" . trim($recno) . "'";
	$result=mysql_query($sql, $dbacc);
		   
	$m_acode = trim($_GET["txtacccode"]);
	   
	$sql="insert recmas (refno, bdate, code, name, heading, barer, naration, amount, type, invno, vatno, inv_date, bea1) values ('".trim($recno)."', '".$_GET["Calendar1"]."', '".$_GET["txtacccode"]."', '".trim($_GET["cmbBarer"])."', '".$_GET["TXT_HEADING"]."', '".$_GET["txt_bea"]."', '".$_GET["TXT_NARA"]."', '".$_GET["TXT_DEBTOT"]."', 'D', '".$_GET["txtINVNO"]."', '".$_GET["txtVATNIO"]."', '".$_GET["DTinv_date"]."', '".$_GET["txtbea1"]."')";
	//echo $sql;
	$result=mysql_query($sql, $dbacc);
		 
    
	$sql="insert ledger (l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_head, l_lmem, comcode, l_flag2, l_yearfl, l_year) values ('".trim($recno)."', '".$_GET["Calendar1"]."', '".trim($m_acode)."', ".$_GET["TXT_DEBTOT"].", 'REC', 'DEB', '".$_GET["TXT_HEADING"]."', '".$_GET["TXT_NARA"]."', '".$_SESSION['company']."', '0', 0, ".date("Y").")";
	$result=mysql_query($sql, $dbacc);
    ...
    $sql="insert audit (userid, username, description, refno, account, amount, flag) values ('userid', 'username', '".$descri."', '".trim($recno)."', '".$m_accno."', ".$m_amount.", 'DRE')";
	//echo $sql;
	$result=mysql_query($sql, $dbacc);  
	...
	while ($row = mysql_fetch_array($result)){
          ...
          if (($m_acode != "") and ($m_amount != 0)) {
		  	$sql1="insert recpaytrn (refno, date, code, amount, flag, NARA) values ('".$recno."', '".$_GET["Calendar1"]."', '".trim($m_acode)."', ".$m_amount.", 'DRE', '".trim($m_nara)."')";
			//echo $sql1;
			$result1=mysql_query($sql1, $dbacc);
             
			$sql1="insert ledger (l_refno, l_date, l_code, l_amount, l_flag, l_flag1, comcode, l_head, l_lmem, l_flag2, l_yearfl, l_year) values ('".trim($recno)."', '".$_GET["Calendar1"]."', '".trim($m_acode)."', ".$m_amount.", 'REC', 'CRE', '".$_SESSION['company']."', '".$_GET["TXT_HEADING"]."', '".$_GET["TXT_NARA"]."', '0', 0, ".date("Y").")";
			$result1=mysql_query($sql1, $dbacc);
             
			$descri = "Receipt Entry" . $m_flag1;
            $sql1="insert audit (userid, username, description, action, refno, account, amount, flag) values ('userid', 'username', '".$descri."', 'Add', '".trim($recno)."', '".$m_accno."', ".$m_amount.", 'DRE')";
			//echo $sql1;
			$result1=mysql_query($sql1, $dbacc);
...
if ($_GET["Command"]=="rec_ent"){
	...
	$result_recmas=mysql_query($sql_recmas, $dbacc);
	if($row_recmas = mysql_fetch_array($result_recmas)){
		
		$ResponseXML .= "<txt_entno><![CDATA[".$_GET["code"]."]]></txt_entno>";
		$ResponseXML .= "<txtacccode><![CDATA[".$row_recmas["code"]."]]></txtacccode>";
		$ResponseXML .= "<txt_bea><![CDATA[".$row_recmas["barer"]."]]></txt_bea>";
		...
		$_SESSION["tmp_no_rec_ent_acc"]=$row_recmas["tmp_no"];
		 ...
		 if ($row_recmas["inv_date"]!="0000-00-00 00:00:00"){
		...
		$result_RSRECEPAYTRN=mysql_query($sql_RSRECEPAYTRN, $dbacc);
		while($row_RSRECEPAYTRN = mysql_fetch_array($result_RSRECEPAYTRN)){
