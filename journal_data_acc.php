...
if($_GET["Command"]=="setamt")
{
	...
	$sql="update tmp_journal1 set amt=".$amt." where   accno='".$_GET["accno"]."' and tmp_no= '".$_SESSION['tmp_no_journal_acc']."'";
	
		$result=mysql_query($sql, $dbacc); 
		
		$sql="select sum(amt) as totamt from tmp_journal1 where  entno='".$_GET["txt_entno"]."' and tmp_no= '".$_SESSION['tmp_no_journal_acc']."'";
		//echo $sql;
		$result=mysql_query($sql, $dbacc); 
...
if($_GET["Command"]=="addchq_cash_rec1")
{
		...
		$TXT_DETAILS=str_replace("~", "&", $_GET["TXT_DETAILS"]);
	 	$TXT_DETAILS=str_replace("&nbsp;", " ", $TXT_DETAILS);	 
	 
		$sql="insert into tmp_journal1(entno, accno, accname, descript, amt, tmp_no) values ('".$_GET["txt_entno"]."', '".$_GET["accno1"]."', '".$_GET["acc_name1"]."', '".$TXT_DETAILS."', ".$_GET["amt1"].", '".$_SESSION["tmp_no_journal_acc"]."')";
		$result=mysql_query($sql, $dbacc);
			...
			$ResponseXML .= "<salesdetails>";
				...
				while ($row = mysql_fetch_array($result)){
					$accno="accno_deb".$i;
					$accname="accname_deb".$i;
					$descript="descript_deb".$i;
					$amt="jouamt_deb".$i;
...
{
	$amt=str_replace(",", "", $_GET["amt"]);
	$sql="update tmp_journal2 set amt=".$amt." where   accno='".$_GET["accno"]."' and tmp_no= '".$_SESSION['tmp_no_journal_acc']."'";
		//echo $sql;
		$result=mysql_query($sql, $dbacc); 
		
		$sql="select sum(amt) as totamt from tmp_journal2 where  entno='".$_GET["txt_entno"]."' and tmp_no= '".$_SESSION['tmp_no_journal_acc']."'";
		//echo $sql;
		$result=mysql_query($sql, $dbacc); 
		$row = mysql_fetch_array($result);
		echo $row["totamt"];
...
if($_GET["Command"]=="addchq_cash_rec2")
{
	...
	//	$result=mysql_query($sql, $dbacc);
		...
		$sql="insert into tmp_journal2(entno, accno, accname, descript, amt, tmp_no) values ('".$_GET["txt_entno"]."', '".$_GET["accno2"]."', '".$_GET["acc_name2"]."', '".$TXT_DETAILS."', ".$_GET["amt2"].", '".$_SESSION["tmp_no_journal_acc"]."')";
		$result=mysql_query($sql, $dbacc);
			...
			$ResponseXML .= "<salesdetails>";
				...
				while ($row = mysql_fetch_array($result)){
					$accno="accno_cre".$i;
					$accname="accname_cre".$i;
					$descript="descript_cre".$i;
					$amt="jouamt_cre".$i;
					
					$ResponseXML .= "<tr>
					<td><input type=\"text\" name=\"".$accno."\" id=\"".$accno."\" onblur=\"set_acc2('".$row["accno"]."', '".$i."');\" value=\"".$row["accno"]."\" /></td>
					<td><input type=\"text\" name=\"".$accname."\" id=\"".$accname."\" value=\"".$row["accname"]."\" /></td>
					<td><input type=\"text\" name=\"".$descript."\" id=\"".$descript."\" value=\"".$row["descript"]."\" />".$row["descript"]."</td>
					<td align=right><input type=\"text\" name=\"".$amt."\" id=\"".$amt."\" onblur=\"setamt_opr2('".$i."');\" value=\"".number_format($row["amt"], 2, ".", "")."\" /></td>
					<td ><img src=\"images/delete_01.png\" width=\"20\" height=\"20\" id=".$row['accno']."  name=".$row['accno']." onClick=\"del_item1('".$row['accno']."');\"></td></tr>";
		...
		$result=mysql_query($sql, $dbacc);
			...
			$ResponseXML .= "<salesdetails>";
				...
				while ($row = mysql_fetch_array($result)){
					...
					<td><input type=\"text\" name=\"".$descript."\" id=\"".$descript."\" value=\"".$row["descript"]."\" />".$row["descript"]."</td>
					<td align=right><input type=\"text\" name=\"".$amt."\" id=\"".$amt."\" onblur=\"setamt_opr2('".$i."');\" value=\"".number_format($row["amt"], 2, ".", "")."\" /></td>
					<td ><img src=\"images/delete_01.png\" width=\"20\" height=\"20\" id=".$row['accno']."  name=".$row['accno']." onClick=\"del_item2('".$row['accno']."');\"></td></tr>";
...
/*
	...
	while ($_GET["mcount"]>$i){
		...
		if($_GET["Command"]=="new_rec")
		{
			...
			$sql="delete from tmp_journal1 where tmp_no = '".$_SESSION["tmp_no_journal_acc"]."'";
			$result=mysql_query($sql, $dbacc);
			
			$sql="delete from tmp_journal2 where tmp_no = '".$_SESSION["tmp_no_journal_acc"]."'";
			$result=mysql_query($sql, $dbacc);
			...
			$ResponseXML .= "<txt_curdate><![CDATA[".date("Y-m-d")."]]></txt_curdate>";
			$ResponseXML .= " </salesdetails>";
				echo $ResponseXML;		
...
if($_GET["Command"]=="save_crec")
{
	...
	$result=mysql_query($sql, $dbacc);
	$row = mysql_fetch_array($result);
	if (($row["datefrom"]<=$_GET["txt_Date"]) and ($_GET["txt_Date"]<=$row["dateto"])){
	} else {
	...
	$result=mysql_query($sql, $dbacc);
	$row = mysql_fetch_array($result);
	if ($row["lock_date"]>=$_GET["txt_Date"]){
	...
	//////////////////
      ...
      if ($m_ok!=""){ exit($m_ok); } 
	   ...
	   //echo "aaa".$_SESSION["txt_stat"];
		// echo "txt_stat-".$_SESSION["txt_stat"];
	   if ($_SESSION["txt_stat"] == "new") {
	   ...
	   if ($_GET["txt_entno"] != "") {
			...
			$sql="delete from ledmas where refno = '" . trim($recno) . "'";
			$result=mysql_query($sql, $dbacc);
			
			$sql="Delete   from ledtrans where refno = '" . trim($recno) . "'";
			$result=mysql_query($sql, $dbacc);
			...
			if ($row = mysql_fetch_array($result)){                     
				...
				$sql1="Delete   from ledger where l_refno = '" . trim($recno) . "'";
	 ...
	 $TXT_DETAILS=str_replace("&nbsp;", " ", $TXT_DETAILS);	 
      ...
      if ($m_ok == "") {
		 ...
		 $sql="Insert into ledmas(refno, bdate, details, rights, comcode, cancel, acyear, tmp_no) Values ('" . trim($recno) . "', '" . $_GET["txt_Date"] . "', '" . trim($TXT_DETAILS) . "', '" . $mUserWrite . "', '" . $_SESSION['company'] . "', '0', '" . date("Y") . "', '".$_SESSION["tmp_no_journal_acc"]."')";
		 //echo $sql;
	     $result=mysql_query($sql, $dbacc);
		 if ($result==false){ exit("Transaction NOT Saved !!!"); }
	  	...
	  	while($row = mysql_fetch_array($result)){       
            ...
            if (($m_acode != "") and ($m_amount != 0)) {
               ...
               if (is_null(trim($m_nara))==false) { $mNara = trim($m_nara); }
			   	...
			   	$sql1="Insert into ledtrans(refno, bdate, code, amount, flag, jobno, nara, comcode, cancel) Values ('" . trim($recno) . "', '" . $_GET["txt_Date"] . "', '" . trim($m_acode) . "', " . $m_amount . ", 'DEB', '" . trim($_GET["Com_job"]) . "', '" . trim($TXT_DETAILS) . "', '" . $_SESSION['company'] . "', '0')";
	  			$result1=mysql_query($sql1, $dbacc);
				if ($result1==false){ exit("Transaction NOT Saved !!!"); }
               ...
               if (is_null(trim($m_nara))==false) { $mNara = trim($m_nara); }
					...
					$sql1="Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, rights, l_flag2, l_flag3, comcode, l_yearfl, l_year, l_month, recdate, user, ent_datetime) Values ('" . trim($recno) . "', '" . $_GET["txt_Date"] . "', '" . trim($m_acode) . "', " . $m_amount . ", 'JOU', 'DEB', '" . trim($TXT_DETAILS) . "', '" . $mUserWrite . "', '0', '" . $Mflag3 . "', '" . $_SESSION['company'] . "', 0, ".date("Y").", '".$l_month."', '".$recdate."', '".$_SESSION['UserName']."', '".date("Y-m-d H:i:s")."')";
					//echo $sql1;
	  				$result1=mysql_query($sql1, $dbacc);
					if ($result1==false){ exit("Transaction NOT Saved !!!"); }
	  	...
	  	while($row = mysql_fetch_array($result)){       
            ...
            if (($m_acode != "") and ($m_amount != 0)) {
               ...
               if (is_null(trim($m_nara))==false) { $mNara = trim($m_nara); }
			   	...
			   	$sql1="Insert into ledtrans(refno, bdate, code, amount, flag, jobno, nara, comcode, cancel) Values ('" . trim($recno) . "', '" . $_GET["txt_Date"] . "', '" . trim($m_acode) . "', " . $m_amount . ", 'CRE', '" . trim($_GET["Com_job"]) . "', '" . trim($TXT_DETAILS) . "', '" . $_SESSION['company'] . "', '0')";
	  			$result1=mysql_query($sql1, $dbacc);
				if ($result1==false){ exit("Transaction NOT Saved !!!"); }
               ...
               if (is_null(trim($m_nara))==false) { $mNara = trim($m_nara); }
					...
					$sql1="Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, rights, l_flag2, l_flag3, comcode, l_yearfl, l_year, l_month, recdate, user, ent_datetime) Values ('" . trim($recno) . "', '" . $_GET["txt_Date"] . "', '" . trim($m_acode) . "', " . $m_amount . ", 'JOU', 'CRE', '" . trim($TXT_DETAILS) . "', '" . $mUserWrite . "', '0', '" . $Mflag3 . "', '" . $_SESSION['company'] . "', 0,  ".date("Y").", '".$l_month."', '".$recdate."', '".$_SESSION['UserName']."', '".date("Y-m-d H:i:s")."')";
					//echo $sql1;
	  				$result1=mysql_query($sql1, $dbacc);
					if ($result1==false){ exit("Transaction NOT Saved !!!"); }
...
if ($_GET["Command"]=="pass_journal_no"){
	...
	if($row_rst = mysql_fetch_array($result_rst)){
		...
		$_SESSION["tmp_no_journal_acc"]=$row_rst["tmp_no"];
		
		$ResponseXML .= "<txt_entno><![CDATA[".trim($_GET["txt_entno"])."]]></txt_entno>";
        $ResponseXML .= "<lab_can><![CDATA[".$row_rst["cancel"]."]]></lab_can>";
		$ResponseXML .= "<txt_Date><![CDATA[".date("Y-m-d", strtotime($row_rst["bdate"]))."]]></txt_Date>";
		...
		while($row_rst1 = mysql_fetch_array($result_rst1)){
           ...
           if (trim($row_rst1["refno"]) == trim($_GET["txt_entno"])) {
              ...
              $TXT_DETAILS=str_replace("~", "&", $row_rst1["nara"]);
	 			  $TXT_DETAILS=str_replace("&nbsp;", " ", $TXT_DETAILS);	 
				  $TXT_DETAILS=str_replace("'", "''", $TXT_DETAILS);	 
              ...
              if ($row_rst1["flag"] == "DEB") {
				...
				$sql="insert into tmp_journal1(entno, accno, accname, descript, amt, tmp_no) values ('".$_GET["txt_entno"]."', '".trim($row_rst1["code"])."', '".$m_acname."', '".trim($TXT_DETAILS)."', ".$row_rst1["amount"].", '".$_SESSION["tmp_no_journal_acc"]."')";
				$result=mysql_query($sql, $dbacc);
				...
				$sql="insert into tmp_journal2(entno, accno, accname, descript, amt, tmp_no) values ('".$_GET["txt_entno"]."', '".trim($row_rst1["code"])."', '".$m_acname."', '".trim($TXT_DETAILS)."', ".$row_rst1["amount"].", '".$_SESSION["tmp_no_journal_acc"]."')";
				$result=mysql_query($sql, $dbacc);
   			...
   			$totamt=0;
				...
				$ResponseXML .= "<chq_table_deb><![CDATA[ <table border=1 cellspacing=\"0\" cellpadding=\"0\" class=\"bcgl1\"><tr height=\"25\">
					<td width=\"100\"  background=\"images/headingbg.gif\">Account No</td>
					<td width=\"200\"  background=\"images/headingbg.gif\">Account Name</td>
					<td width=\"300\"  background=\"images/headingbg.gif\">Description</td>
					<td width=\"140\"  background=\"images/headingbg.gif\">Amount</td>
				...
				$ResponseXML .= "   </table>]]></chq_table_deb>";
				$ResponseXML .= "<totamt_deb><![CDATA[".$totamt."]]></totamt_deb>";		
				
				
				
				$totamt=0;
			
				$ResponseXML .= "<chq_table_cre><![CDATA[ <table border=1 cellspacing=\"0\" cellpadding=\"0\" class=\"bcgl1\"><tr height=\"25\">
					<td width=\"100\"  background=\"images/headingbg.gif\">Account No</td>
					<td width=\"200\"  background=\"images/headingbg.gif\">Account Name</td>
					<td width=\"300\"  background=\"images/headingbg.gif\">Description</td>
					<td width=\"140\"  background=\"images/headingbg.gif\">Amount</td>
				...
				$ResponseXML .= "   </table>]]></chq_table_cre>";
				$ResponseXML .= "<totamt_cre><![CDATA[".$totamt."]]></totamt_cre>";		
...
if ($_GET["Command"]=="set_acc"){
    ...
    if($row_rst = mysql_fetch_array($result_rst)){	
		...
		$sql1="update tmp_journal1 set accno='".trim($row_rst["c_code"])."', accname='".trim($row_rst["c_name"])."', descript='".trim($_GET["TXT_NARA"])."' where entno='".trim($_GET["txt_entno"])."' and accno='".$_GET["old_accno"]."'";
		$result1=mysql_query($sql1, $dbacc);
...
if ($_GET["Command"]=="set_acc2"){
    ...
    if($row_rst = mysql_fetch_array($result_rst)){	
		...
		$sql1="update tmp_journal2 set accno='".trim($row_rst["c_code"])."', accname='".trim($row_rst["c_name"])."', descript='".trim($_GET["TXT_NARA"])."' where entno='".trim($_GET["txt_entno"])."' and accno='".$_GET["old_accno"]."'";
		$result1=mysql_query($sql1, $dbacc);
...
if ($_GET["Command"]=="delete_rec"){
	...
	//if (strtotime($_GET["txt_Date"])==strtotime(date("Y-m-d"))){
	...
	/*} else {
		$ResponseXML = "Sorry Can't Cancel.... please check reciept date";
	}*/
