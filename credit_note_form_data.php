...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
	...
	/*		$sql="Select * from tmp_army_no where edu= '".$_GET['edu']."'";
		...
		/*	$sql="Select CAS_INV_NO_m from invpara";
			...
			$lenth=strlen($tmpinvno);
			$invno=trim("CFRM/ ").substr($tmpinvno, $lenth-7);
...
if ($_GET["Command"]=="select_list"){
	...
	if ($_GET["mstatus"]=="all"){
		...
		$sql = "select Refno, Code, Amount   from s_crnfrm where cancell = '0' and Lock1='1' and flag = 'ACRN' order BY Refno desc limit 50";
							...
							echo "<tr>               
                              <td onclick=\"invno1('".$row['Refno']."', '".$_GET['stname']."');\">".$row['Refno']."</a></td>
                              <td onclick=\"invno1('".$row['Refno']."', '".$_GET['stname']."');\">".$row["Code"]."</a></td>
                              <td onclick=\"invno1('".$row['Refno']."', '".$_GET['stname']."');\">".$row['Amount']."</a></td>
   ...
   // Do While MSFlexGrid1.TextMatrix(i, 1) <> ""
		...
		if ($row_rscrn = mysql_fetch_array($result_rscrn)){
        	...
        	if ($row_rssalma = mysql_fetch_array($result_rssalma)){
				...
				$sql_tmpinst= " insert into tmp_credit_note_form (crn_form_no, Inv_date, inv_no, Amount, disc, Qty,  Brands) values ('".$_GET["txtrefno"]."', '".$row_rssalma['SDATE']."', '".$_GET["invno"]."', ".$row_rssalma['GRAND_TOT'].", ".$row_rssalma['DIS'].", ".$mqty.", '".$row_rssalma['Brand']."' )";
				//echo $sql_tmpinst;
        		$result_tmpinst =$db->RunQuery($sql_tmpinst);
...
if($_GET["Command"]=="save_item")
{
  ...
  if (($_GET["txt_cuscode"] != "") and ($_GET["mcou"] > 0)){
    ...
    if($row_rscrnfrm = mysql_fetch_array($result_rscrnfrm)){	
		...
		$sql1= "insert into s_crnfrm (Refno, Sdate, Code, Mon, Amount, Remark, Sal_ex, Flag) Values('" . $mrefno . "','" . $_GET["DTPicker1"] . "', '" . trim($_GET["txt_cuscode"]) . "', '" . $_GET["MonthView1"] . "', '" . $mamount . "','" . trim($_GET["txtremark"]) . "','" . $_GET["Com_rep"] . "','ACRN')";
		$result1 =$db->RunQuery($sql1);
