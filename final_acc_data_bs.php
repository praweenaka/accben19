...
if ($_GET["Command"] == "save_mettrix_text") {
    ...
    $sql = "delete from acc_cel_cal_bs where mrow=" . $_SESSION["i"] . " and mcol=" . $_SESSION["j"] . "";
    $result = $db->RunQuery($sql);

    $sql = "insert into acc_cel_cal_bs(mrow, mcol, description, cell_type, mbold, munder) values ('" . $_SESSION["i"] . "', '" . $_SESSION["j"] . "', '" . $_GET["sheet_desc"] . "', 'text', '" . $_GET["mbold"] . "', '" . $_GET["munder"] . "')";
...
if ($_GET["Command"] == "save_mettrix_cell_r") {
    ...
    $sql = "insert into acc_cel_cal_bs(mrow, mcol, mrow1, mcol1, mrow2, mcol2, mrow3, mcol3, mrow4, mcol4, opr, cell_type, mbold, munder) values (" . $_SESSION["i"] . ", " . $_SESSION["j"] . ", " . $row1 . ", " . $col1 . ", " . $row2 . ", " . $col2 . ", " . $row3 . ", " . $col3 . ", " . $row4 . ", " . $col4 . ", '" . $opr . "', 'opr', '" . $_GET["mbold"] . "', '" . $_GET["munder"] . "')";
    //echo $sql;
    $result = $db->RunQuery($sql);
...
if ($_GET["Command"] == "save_mettrix_coll_acc") {
    ...
    $sql1 = "delete from acc_account_data_bs where mrow=" . $_SESSION["i"] . " and mcol=" . $_SESSION["j"] . " and (acc_type= 'acc_front' or acc_type= 'acc_rear' or acc_type= 'text' or acc_type= 'opr')";
    $result1 = $db->RunQuery($sql1);
    ...
    if (($_GET["txt_acc"] != "") and ( $_GET["val_acc"] != 0)) {
        $sql1 = "insert into acc_account_data_bs(mrow, mcol, add_less, acc_code, acc_name, acc_type, acc_val) values (" . $_SESSION["i"] . ", " . $_SESSION["j"] . ", '" . $opr . "', '" . $row["c_code"] . "', '" . $_GET["txt_acc"] . "', 'acc_front', " . $_GET["val_acc"] . ")";
        echo $sql1;
        $result1 = $db->RunQuery($sql1);
    }
    ...
    if (($_GET["txt_acc_last"] != "") and ( $_GET["val_acc_last"] != 0)) {
        $sql1 = "insert into acc_account_data_bs(mrow, mcol, add_less, acc_code, acc_name, acc_type, acc_val) values (" . $_SESSION["i"] . ", " . $_SESSION["j"] . ", '" . $opr . "', '" . $row["c_code"] . "', '" . $_GET["txt_acc_last"] . "', 'acc_rear', " . $_GET["val_acc_last"] . ")";
        echo $sql1;
        $result1 = $db->RunQuery($sql1);
    }

    $sql1 = "update acc_cel_cal_bs set set mbold='" . $_GET["mbold"] . "', munder='" . $_GET["munder"] . "' where mrow=" . $_SESSION["i"] . " and mcol=" . $_SESSION["j"];
    //echo $sql1;
    $result1 = $db->RunQuery($sql1);

    $acc_list = "";
    $sql = "select * from acc_cel_cal_bs where mrow=" . $_SESSION["i"] . " and mcol=" . $_SESSION["j"] . " and cell_type='acc'";
    $result = $db->RunQuery($sql);
...
if ($_GET["Command"] == "del_item") {
    ...
    $sql1 = "delete from acc_account_data_bs where id='" . $_GET["c_code"] . "'";
    $result1 = $db->RunQuery($sql1);
    ...
    $sql1 = "select * from acc_account_data_bs where mrow='" . $_GET["mrow"] . "' and mcol='" . $_GET["mcol"] . "'";
...
if ($_GET["Command"] == "loadnt") {
    ...
    $sql = "select * from acc_account_data_bs where mrow='" . $_GET["row"] . "' and mcol ='4'";
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        if (mysql_num_rows($resultr) > 0) {
            while ($rstr = mysql_fetch_array($resultr)) {
                ...
                $sql = "select * from pnlpos_bs where l_code = '" . $rstr['c_code'] . "'";
...
if ($_GET["Command"] == "save_note") {
    ...
    while ($i >= $j) {
        ...
        $sql = "delete from pnlpos_bs where l_code ='" . $_GET[$test_desc] . "'";
        $result = $db->RunQuery($sql);
        ...
        $sql = "insert into pnlpos_bs  (l_code,pos)  values ('" . $_GET[$test_desc] . "','" . $mdt . "')";
        $result = $db->RunQuery($sql);
