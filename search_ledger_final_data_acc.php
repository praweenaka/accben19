...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
						   ...
						   $letters = $_GET['cusno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select cheque_no, che_amount, che_date, bank, refno, CUS_NAME, ID, ret_refno  from s_invcheq where cheque_no like '$letters%' ORDER BY che_date limit 50";
...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="cusno"){
						   		$letters = $_GET['cusno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from lcodes where c_code like '$letters%' ORDER BY c_code limit 100";
								$result=mysql_query($sql, $dbacc);
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from lcodes where   c_name like '$letters%' ORDER BY c_code limit 100";
								$result=mysql_query($sql, $dbacc);
...
if ($_GET["Command"]=="search_ledger_sel"){
...
if ($_GET["Command"]=="search_journal"){
...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
						...
						//	if ($_GET["mstatus"]=="cusno"){
						   		$letters = $_GET['cusno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="Select refno, bdate, details from ledmas where refno  like '$letters%' ORDER BY refno desc limit 50";
						...
						/*	} else if ($_GET["mstatus"]=="customername"){
							...
							$ResponseXML .= "<tr>               
                              ...
                              <td onclick=\"journal('$cuscode', '$stname');\">".date("Y-m-d", strtotime($row['bdate']))."</a></td>
...
if ($_GET["Command"]=="final_acc"){
	...
	if($row = mysql_fetch_array($result)){
            ...
            if ($_GET['stname']=="final_acc") {
		$sql1="insert into acc_cel_cal(mrow, mcol, cell_type) values (".$_SESSION["i"].", ".$_SESSION["j"].", 'acc')";
		$result1=mysql_query($sql1, $dbacc);
		
		$sql1="delete from acc_account_data where mrow=".$_SESSION["i"]." and mcol=".$_SESSION["j"]." and acc_code= '".$row["c_code"]."'";
		$result1=mysql_query($sql1, $dbacc);
		...
		$sql1="insert into acc_account_data(mrow, mcol, add_less, acc_code, acc_name, acc_type) values (".$_SESSION["i"].", ".$_SESSION["j"].", '".$opr."', '".$row["c_code"]."', '".$row["c_name"]."', 'acc')";
		$result1=mysql_query($sql1, $dbacc);
            } else {
               		$sql1="insert into acc_cel_cal_bs(mrow, mcol, cell_type) values (".$_SESSION["i"].", ".$_SESSION["j"].", 'acc')";
		$result1=mysql_query($sql1, $dbacc);
		
		$sql1="delete from acc_account_data_bs where mrow=".$_SESSION["i"]." and mcol=".$_SESSION["j"]." and acc_code= '".$row["c_code"]."'";
		$result1=mysql_query($sql1, $dbacc);
		...
		$sql1="insert into acc_account_data_bs(mrow, mcol, add_less, acc_code, acc_name, acc_type) values (".$_SESSION["i"].", ".$_SESSION["j"].", '".$opr."', '".$row["c_code"]."', '".$row["c_name"]."', 'acc')";
		$result1=mysql_query($sql1, $dbacc); 
            }   
	...
	$ResponseXML .= "<acc_table><![CDATA[<table width=\"415\" border=\"1\">";
	$ResponseXML .= "<tr>
	...
	$sql="select * from acc_cel_cal where mrow=".$_SESSION["i"]." and mcol=".$_SESSION["j"]."";
        ...
        $sql="select * from acc_cel_cal_bs where mrow=".$_SESSION["i"]." and mcol=".$_SESSION["j"]."";     
         }
	$result=mysql_query($sql, $dbacc);
	if($row = mysql_fetch_array($result)){	
			...
			$sql1="select * from acc_account_data where mrow=".$_SESSION["i"]." and mcol=".$_SESSION["j"]."";
                        ...
                        $sql1="select * from acc_account_data_bs where mrow=".$_SESSION["i"]." and mcol=".$_SESSION["j"]."";    
                        }
			$result1=mysql_query($sql1, $dbacc); 
			while($row1 = mysql_fetch_array($result1)){	
      			$ResponseXML.="<tr><td>".$row1["acc_code"]."</td>";
				$ResponseXML.="<td>".$row1["acc_name"]."</td>
				<td ><img src=\"images/delete_01.png\" width=\"20\" height=\"20\" id=".$row['accno']."  name=".$row['accno']." onClick=\"del_item('".$row['mrow']."', '".$row['mcol']."', '".$row['acc_code']."');\"></td></tr>";
			}
...
if ($_GET["Command"]=="pass_ledger_sel"){
	...
	if($row = mysql_fetch_array($result)){							
		...
		$ResponseXML .= "<cat><![CDATA[".trim($row['cat'])."]]></cat>";
		$ResponseXML .= "<c_group><![CDATA[".trim($row['c_group'])."]]></c_group>";
		$ResponseXML .= "<c_nod_add><![CDATA[".trim($row['c_nod_add'])."]]></c_nod_add>";
		$ResponseXML .= "<c_date><![CDATA[".trim($row['c_date'])."]]></c_date>";
		$ResponseXML .= "<comments><![CDATA[".trim($row['comments'])."]]></comments>";
		
		//if ($_SESSION['dev']=="1"){
			$ResponseXML .= "<c_opbal1><![CDATA[".trim($row['c_opbal1'])."]]></c_opbal1>";
		//} else {
		//	$ResponseXML .= "<c_opbal1><![CDATA[]]></c_opbal1>";
		//}	
		
		$sql_b="select * from bankmaster where bm_code='".$_GET["ledgno"]."'";
		$result_b=mysql_query($sql_b, $dbacc);
		if ($row_b = mysql_fetch_array($result_b)){		
			$ResponseXML .= "<bm_chno><![CDATA[".trim($row_b['bm_chno'])."]]></bm_chno>"; 
		} else {
			$ResponseXML .= "<bm_chno><![CDATA[]]></bm_chno>"; 
		}	
...
if ($_GET["Command"]=="pass_cash_rec"){
	...
	$sql="select c_code, c_name from lcodes where c_code='".$_GET["ledgno"]."' and c_group='D'";
	$result=mysql_query($sql, $dbacc);
	if($row = mysql_fetch_array($result)){							
		$ResponseXML .= "<c_code><![CDATA[".$row['c_code']."]]></c_code>";
		$ResponseXML .= "<c_name><![CDATA[".$row['c_name']."]]></c_name>";
