    ...
    <body>
        ...
        while ($rst1 = mysqli_fetch_array($result1)) {
            ...
            while ($rstr = mysqli_fetch_array($resultr)) {
                ...
                $sql = "select * from acc_account_data_bs where (acc_code = '" . $rstr['c_code'] . "' or acc_code ='" . $rstr['paccno'] . "') and mrow='" . $_POST["row"] . "' and mcol ='4'";
...
//             
        ...
        $sql = "select * from pnl_notes_data_bs where sdate <='" . $_POST['dtto'] . "'and mrow = '" . $_POST["row"] . "' order by sdate desc";
        ...
        $sql = "select * from acc_cel_cal_bs where mrow='" . $_POST["row"] . "' and mcol ='3'";
        ...
        $sql = "select * from acc_cel_cal_bs where mrow='" . $_POST["row"] . "' and mcol ='1'";
        $result = mysqli_query($dbacc, $sql);
        $rst = mysqli_fetch_array($result);
        ...
        /*
          ...
          $sql = "select * from acc_account_data_bs where mrow='" . $_POST["row"] . "' and mcol ='4'";
          $result1 = mysqli_query($dbacc, $sql);
          while ($rst1 = mysqli_fetch_array($result1)) {
          if ($rst1['actype']== "D") {
...
//        }
        ...
        $sql = "select * from pnlpos_bs where mrow = '" . $_POST["row"] . "' order by pos ";
        $result1 = mysqli_query($dbacc, $sql);
        $numrows = mysqli_num_rows($result1) + 1;
        $i = 1;

        if ($numrows == 1) {
            $sql = "select l_code,l_name from view_pnlnotes_bs group by  l_code,l_name  order by pos,l_code";
            $result1 = mysqli_query($dbacc, $sql);
            $numrows = mysqli_num_rows($result1) + 1;
        }

        $sql = "select * from pnlpos_bs where mrow = '" . $_POST["row"] . "'   and noteP <> ''  order by pos ";
        $result1 = mysqli_query($dbacc, $sql);
        ...
        if ($numrows1 > 1) {
            ...
            while ($i < ($numrows + $numrows1)) {
                ...
                $sql = "select * from pnlpos_bs where mrow = '" . $_POST["row"] . "' and pos = '" . $i . "' and noteP <> ''  order by pos ";
                ...
                if ($msign != "") {
                    ...
                    if ($_POST['type'] != 'date') {
                        ...
                        $mamo = 0;
                        $sql = "select  sum(l_amount*subtot) as l_amo from view_pnlnotes_bs where atype<> 'E'  ";
                        $resultm1 = mysqli_query($dbacc, $sql);
                        if ($rstm1 = mysqli_fetch_array($resultm1)) {
                        ...
                        $sql = "select  (l_amount*subtot) as  l_amount from view_pnlnotes_bs  where   atype = 'E'  and l_code <> '" . $mcode . "' order by l_date  ";
                        $resultm1 = mysqli_query($dbacc, $sql);
                        if ($rstm1 = mysqli_fetch_array($resultm1)) {
                            $mamo = $mamo + $rstm1['l_amount'];
                        }
                ...
                $sql = "select l_code,l_name from view_pnlnotes_bs where pos = '" . $i . "'  group by  l_code,l_name  order by pos,l_code";
                $resultr = mysqli_query($dbacc, $sql);
                ...
                if ($msign != "") {
                    ...
                    while ($rstm = mysqli_fetch_array($resultm)) {
                        ...
                        $sql = "select  sum(l_amount*subtot) as l_amo from view_pnlnotes_bs  where   l_mon = '" . $rstm['l_mon'] . "' and l_year='" . $rstm['l_year'] . "' and atype<> 'E'";
                        $resultm1 = mysqli_query($dbacc, $sql);
                        $mamo = 0;
                        if ($rstm1 = mysqli_fetch_array($resultm1)) {
                            $mamo = $rstm1['l_amo'];
                        ...
                        $ldate = $rstm['l_year'] . "-" . $rstm['l_mon'] . "-01";
                        $ldate = date("Y-m-t", strtotime($ldate));
                        $sql = "select  (l_amount*subtot) as  l_amount from view_pnlnotes_bs  where  l_mon = '" . $rstm['l_mon'] . "' and l_year='" . $rstm['l_year'] . "'   and atype = 'E'  and l_code = '" . $mcode . "'";
                    ...
                    if ($_POST['type'] != 'date') {
                        ...
                        $sql = "select  (l_amount*subtot) as  l_amount from view_pnlnotes_bs  where  atype = 'E'  and l_code <> '" . $mcode . "'";
                        if ($mcode != "T1") {
                            $sql .= "  order by l_date ";
                        } else {
                            $sql .= "  order by l_date desc";
