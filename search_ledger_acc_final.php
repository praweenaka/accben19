...
<script language="JavaScript" src="js/search_ledger_final_acc.js"></script>
<style type="text/css">
...
<tr>
					<?php 
	  				$stname = $_GET["stname"];
					?>
                             <td width="122"  background="images/headingbg.gif" ><input type="text" size="20" name="cusno" id="cusno" value="" class="txtbox" tabindex="1" onkeyup="<?php	echo "update_cust_list('$stname')"; ?>" onkeypress="ledgno('$cuscode', '$stname');"/></td>
      <td width="603"  background="images/headingbg.gif" ><input type="text" size="70" name="customername" id="customername" value="" class="txtbox" onkeyup="<?php echo "update_cust_list('$stname')"; ?>"/></td>
                ...
                <div id="filt_table" class="CSSTableGenerator">  <table width="735" border="0" class=\"form-matrix-table\">
						...
						} else if ($_GET["stname"]=="ledger_sel"){
							$sql="select c_code, c_name from lcodes ORDER BY c_code limit 50";
						} else if ($_GET["stname"]=="ledger"){
							$sql="select c_code, c_name from lcodes ORDER BY c_code limit 50";
						} else {
							$sql="select c_code, c_name from lcodes where c_group='P' ORDER BY c_code limit 50";
						}	
							$stname = $_GET["stname"];
