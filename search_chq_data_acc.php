...
if ($_GET["Command"]=="set_vatno"){
	include_once("connection.php");
	
	
	$sql="Select bea1, vatno from paymas where bea1 = '" . trim($_GET["txtvatbea"]) . "' order by vatno desc";
...
if ($_GET["Command"]=="pass_chqno_vat_barer"){
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$ResponseXML .= "<select name=\"txtvatbea\" id=\"txtvatbea\"  class=\"text_purchase3\" onchange=\"set_vatno();\">";
			
		//	$ResponseXML .= "<option value=\"\"></option>	";
		//	$ResponseXML .= "<option value=\"".$_GET["custno"]."\">".$_GET["custno"]."</option>	";
			
			$bea1="";
			
		/*	$sql="select * from paymas  where barer='".$_GET["custno"]."' and bea1!='' and vatno!='' order by bdate desc";
			$result=mysql_query($sql, $dbacc);
			while($row = mysql_fetch_array($result)){
				$bea1=$row["bea1"]	;
				$ResponseXML .= "<bea1><![CDATA[".$row["bea1"]."]]></bea1>";
			...
			*/
			...
			$custno=str_replace("~", "&", $_GET['custno']);
			
			$sql="select * from che_customers  where chepay='".$custno."' and vat!='' and active='1' order by id desc";
			//echo $sql;
			$result=mysql_query($sql, $dbacc);
			while($row = mysql_fetch_array($result)){
				$bea1=$row["chepay"];
				$ResponseXML .= "<bea1><![CDATA[".$row["chepay"]."]]></bea1>";
				$ResponseXML .= "<vatno><![CDATA[".$row["vat"]."]]></vatno>";
...
if ($_GET["Command"]=="pass_chqno_vat_barer1"){
	...
	$ResponseXML .= "<salesdetails>";
		...
		$bea1="";
			...
			$sql="select * from che_customers  where chepay='".$custno."' ";
			//echo $sql;
			$result=mysql_query($sql, $dbacc);
			while($row = mysql_fetch_array($result)){
				$bea1=$row["chepay"];
...
if ($_GET["Command"]=="pass_chqno_chqno"){
	...
	$sql="select * from s_invcheq where cheque_no='" . $_GET["chqno"]."' and (ret_refno = '0' or ret_refno = '1')"; 
	$result=mysql_query($sql, $dbinv);
	if($row = mysql_fetch_array($result)){							
		$ResponseXML .= "<cheque_no><![CDATA[".$row['cheque_no']."]]></cheque_no>";
		$ResponseXML .= "<che_date><![CDATA[".$row['che_date']."]]></che_date>";
