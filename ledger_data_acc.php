...
if($_GET["Command"]=="save_crec")
{
	...
	if (trim($_GET["txtOpenBal"])==""){
		$txtOpenBal=0;
	} else {
		$txtOpenBal=$_GET["txtOpenBal"];
	}
	if (trim($_GET["txtOpenBal1"])==""){
		$txtOpenBal1=0;;
	} else {
		$txtOpenBal1=$_GET["txtOpenBal1"];
	}
	...
	if ($row_rst = mysql_fetch_array($result_rst)){
		...
		if ($_SESSION['dev']=="1"){
			$sql_lcode="update lcodes set   c_opbal=" . $txtOpenBal . ", c_opbal1=" . $txtOpenBal1 . ", cat='" . $mm . "', c_code='" . trim($_GET["txtAccCode"]) . "', c_name='" . trim($_GET["txtAccName"]) . "', c_type='" . $optAccType . "',  c_add1= '" . trim($_GET["txtAdd1"]) . "', c_add2='" . trim($_GET["txtAdd2"]) . "', c_date='" . $_GET["dtpOpenDate"] . "', c_nod_add='" . trim($_GET["txtLinkNo"]) . "', paccno='" . trim($_GET["txtLinkNo"]) . "', bud1=" . $_GET["jan"] . ", bud2=" . $_GET["feb"] . ", bud3=" . $_GET["mar"] . ", bud4=" . $_GET["apr"] . ", bud5=" . $_GET["may"] . " , bud6=" . $_GET["jun"] . ",bud7=" . $_GET["jul"] . " , bud8=" . $_GET["aug"] . ",bud9=" . $_GET["sep"] . ",bud10=" . $_GET["oct"] . ", bud11=" . $_GET["nov"] . ", bud12=" . $_GET["dec"] . " ,code1='" . trim($_GET["txtcode"]) . "', c_group='".$_GET["level"]."', comments='".$_GET["textcomments"]."' where c_code='" . trim($_GET["txtAccCode"]) . "'";
		echo $sql_lcode;
			
		} else {
			$sql_lcode="update lcodes set   c_opbal=" . $txtOpenBal . ", cat='" . $mm . "', c_code='" . trim($_GET["txtAccCode"]) . "', c_name='" . trim($_GET["txtAccName"]) . "', c_type='" . $optAccType . "',  c_add1= '" . trim($_GET["txtAdd1"]) . "', c_add2='" . trim($_GET["txtAdd2"]) . "', c_date='" . $_GET["dtpOpenDate"] . "', c_nod_add='" . trim($_GET["txtLinkNo"]) . "', paccno='" . trim($_GET["txtLinkNo"]) . "', bud1=" . $_GET["jan"] . ", bud2=" . $_GET["feb"] . ", bud3=" . $_GET["mar"] . ", bud4=" . $_GET["apr"] . ", bud5=" . $_GET["may"] . " , bud6=" . $_GET["jun"] . ",bud7=" . $_GET["jul"] . " , bud8=" . $_GET["aug"] . ",bud9=" . $_GET["sep"] . ",bud10=" . $_GET["oct"] . ", bud11=" . $_GET["nov"] . ", bud12=" . $_GET["dec"] . " ,code1='" . trim($_GET["txtcode"]) . "', c_group='".$_GET["level"]."', comments='".$_GET["textcomments"]."' where c_code='" . trim($_GET["txtAccCode"]) . "'";
		}	
		...
		if ($_SESSION['dev']=="1"){
			$sql_lcode="Insert Into lcodes(c_code, c_name, c_type,   c_add1, c_add2, c_date, cat,  paccno, bud1, bud2, bud3, bud4, bud5, bud6, bud7, bud8, bud9, bud10, bud11, bud12, c_opbal, c_opbal1, code1, c_group, comments) Values ('" . trim($_GET["txtAccCode"]) . "', '" . trim($_GET["txtAccName"]) . "', '" . $optAccType . "',  '" . trim($_GET["txtAdd1"]) . "', '" . trim($_GET["txtAdd2"]) . "', '" . $_GET["dtpOpenDate"] . "', '" . $mm . "', '" . $PAccNo . "'," . $_GET["jan"] . " ," . $_GET["feb"] . "," . $_GET["mar"]. "," . $_GET["apr"] . "," . $_GET["may"] . "," . $_GET["jun"] . "," . $_GET["jul"] . "," . $_GET["aug"] . "," . $_GET["sep"] . "," . $_GET["oct"] . "," . $_GET["nov"] . "," . $_GET["dec"] . "," . $txtOpenBal . "," . $txtOpenBal1 . ",'" . trim($_GET["txtcode"]) . "', '".$_GET["level"]."', '".$_GET["textcomments"]."')";
			$result_lcode=mysql_query($sql_lcode, $dbacc);
		echo $sql_lcode;
		} else {
			$sql_lcode="Insert Into lcodes(c_code, c_name, c_type,   c_add1, c_add2, c_date, cat,  paccno, bud1, bud2, bud3, bud4, bud5, bud6, bud7, bud8, bud9, bud10, bud11, bud12, c_opbal, code1, c_group, comments) Values ('" . trim($_GET["txtAccCode"]) . "', '" . trim($_GET["txtAccName"]) . "', '" . $optAccType . "',  '" . trim($_GET["txtAdd1"]) . "', '" . trim($_GET["txtAdd2"]) . "', '" . $_GET["dtpOpenDate"] . "', '" . $mm . "', '" . $PAccNo . "'," . $_GET["jan"] . " ," . $_GET["feb"] . "," . $_GET["mar"]. "," . $_GET["apr"] . "," . $_GET["may"] . "," . $_GET["jun"] . "," . $_GET["jul"] . "," . $_GET["aug"] . "," . $_GET["sep"] . "," . $_GET["oct"] . "," . $_GET["nov"] . "," . $_GET["dec"] . "," . $txtOpenBal . ", '" . trim($_GET["txtcode"]) . "', '".$_GET["level"]."', '".$_GET["textcomments"]."')";
			echo $sql_lcode;
			$result_lcode=mysql_query($sql_lcode, $dbacc);
		}
	...
	if ($mm=="B"){
		...
		$sql_ban_mas="delete from bankmaster where bm_code = '".$_GET["txtAccCode"]."'";
		$result_ban_mas=mysql_query($sql_ban_mas, $dbacc);
		
		$sql_ban_mas="insert into bankmaster(bm_code, bm_bank, bm_opbal, bm_opdate, bm_chno)values('".$_GET["txtAccCode"]."', '".$_GET["txtAccName"]."', ".$_GET["txtOpenBal"].", '".$_GET["dtpOpenDate"]."', '".$_GET["chqno"]."')";
		$result_ban_mas=mysql_query($sql_ban_mas, $dbacc);
		
		$sql_ban_bal="select * from bank_bal where code='" . trim($_GET["txtAccCode"]) . "' and byear='".date("Y", strtotime($_GET["dtpOpenDate"]))."' and bmonth='".date("m", strtotime($_GET["dtpOpenDate"]))."'";
		$result_ban_bal=mysql_query($sql_ban_bal, $dbacc);
		if ($row_ban_bal = mysql_fetch_array($result_ban_bal)){
			$sql_lcode_update="update bank_bal set   close_balance=" . $txtOpenBal . ",  bdate='" . $_GET["dtpOpenDate"] . "' where code='" . trim($_GET["txtAccCode"]) . "' and byear='".date("Y", strtotime($_GET["dtpOpenDate"]))."' and bmonth='".date("m", strtotime($_GET["dtpOpenDate"]))."'";
			echo $sql_lcode_update;
			$result_lcode_update=mysql_query($sql_lcode_update, $dbacc);
			
		} else {
			$sql_lcode_update="insert into bank_bal (code, bank, bdate, close_balance, bmonth, byear) values ('".$_GET["txtAccCode"]."', '".$_GET["txtAccName"]."', '".$_GET["dtpOpenDate"]."', " . $txtOpenBal . ", '".date("m", strtotime($_GET["dtpOpenDate"]))."', '".date("Y", strtotime($_GET["dtpOpenDate"]))."')";
			echo $sql_lcode_update;
			$result_lcode_update=mysql_query($sql_lcode_update, $dbacc);
...
if ($_GET["Command"]=="update_chq"){
			
		if ($_GET["Check2"]=="true"){	
			$sql_rst = "update bankmaster set bm_chno='".$_GET["chqno"]."'  where bm_code='" . trim($_GET["txtAccCode"]) . "'";
			$result_rst=mysql_query($sql_rst, $dbacc);
			echo "Cheque number updated";
		}	else {
			echo "Can't update";
		}	
}
