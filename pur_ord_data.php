...
if($_GET["Command"]=="save_item")
{
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$_GET["CHECK_USER"] = false ;       //check user permission again
			...
			$sql="select * from s_ordmas where tmp_no='".$_SESSION["tmp_no_purord"]."'";
			$result =$db->RunQuery($sql);
			//echo $sql;
			if($row = mysql_fetch_array($result)){
				$sql1="delete from s_ordmas where tmp_no='".$_SESSION["tmp_no_purord"]."'";
				$result1 =$db->RunQuery($sql1);	
				
				$sql1="delete from s_ordtrn where tmp_no='".$_SESSION["tmp_no_purord"]."'";
				$result1 =$db->RunQuery($sql1);	
			} else {
				$sql="Select ORD_NO from invpara";
				$result =$db->RunQuery($sql);
