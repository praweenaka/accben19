	...
	while ($_GET["mcount"]>$i){
		...
		/*	$sql="Select CAS_INV_NO_m from invpara";
			...
			if ($_SESSION['dev'] == "0") {
				...
				$sql="update tmp_inv_data set INV_RECNO=INV_RECNO+1";
				$result =$db->RunQuery($sql);
 			}
			...
			if ($_SESSION['dev'] == "1") {
				...
				$sql="update tmp_inv_data set INV_RECNO_M=INV_RECNO_M+1";
				$result =$db->RunQuery($sql);
...
if($_GET["Command"]=="save_crec")
{
	...
	$row = mysql_fetch_array($result);
	if (($row["datefrom"]<=$_GET["invdate"]) and ($_GET["invdate"]<=$row["dateto"])){
	} else {
