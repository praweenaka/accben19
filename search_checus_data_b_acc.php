...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
                           ...
                           <td width=\"100\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">VAT No</font></td>
   							</tr>";
                           
						/*	if ($_GET["mstatus"]=="cusno"){*/
						   		$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								if ($_GET["stname"]=="cheque_pay_edit"){
									$sql="select * from che_customers where   chepay like '$letters%' ORDER BY chepay limit 50";
								} else {
									$sql="select * from che_customers where  active='1' and chepay like '$letters%' ORDER BY chepay limit 50";
								}	
								//echo $_GET["stname"];
							...
							/*} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from che_customers where cat='B' and c_name like '$letters%' ORDER BY c_code limit 50";
								$result=mysql_query($sql, $dbacc);
				...
				if (($_GET["stname"]=="cheque_pay") or ($_GET["stname"]=="cheque_pay1") or ($_GET["stname"]=="cheque_pay2") or ($_GET["stname"]=="cheque_pay3")){
						...
						while($row = mysql_fetch_array($result)){
							...
							$ResponseXML .= "<tr>               
							  ...
							  <td onclick=\"che_cus('$cuscode', '$stname');\">".$row['vat']."</a></td>";
                           ...
                           $ResponseXML .=  "</tr>";
							...
							$ResponseXML .= "<tr>               
							 ...
							 if  ($row['active']=="1"){
							  $ResponseXML .= "<td onclick=\"edit_ch_cus('".$row['chepay']."', '".$i."');\"><input type=\"checkbox\" name=\"".$checkbox."\" id=\"".$checkbox."\" checked /></td>";
							  } else {
							  	$ResponseXML .= "<td onclick=\"edit_ch_cus('".$row['chepay']."', '".$i."');\"><input type=\"checkbox\" name=\"".$checkbox."\" id=\"".$checkbox."\" /></td>";
							  }
